...
function createUnitRegistry() {
  ...
  addSimple('in', 'in', 0.0254, { length: 1 }, ['inch', 'inches']);
  ...
  addSimple('tonne', 't', 1000000, { mass: 1 }, ['t', 'tonne', 'tonnes', 'metric ton']);
  ...
  addSimple('m2', 'm²', 1, { length: 2 }, ['m2', 'sqm']);
  addSimple('cm2', 'cm²', 0.0001, { length: 2 }, ['cm2']);
  addSimple('ft2', 'ft²', 0.09290304, { length: 2 }, ['ft2']);
  addSimple('m3', 'm³', 1, { length: 3 }, ['m3', 'cbm']);
  addSimple('l', 'L', 0.001, { length: 3 }, ['l', 'liter', 'liters', 'litre', 'litres']);
  addSimple('ml', 'mL', 0.000001, { length: 3 }, ['ml', 'milliliter', 'milliliters']);
...
function computeRollup(context, mode) {
  ...
  for (let i = context.lineValues.length - 1; i >= 0; i -= 1) {
    ...
    if (line?.value && isNumericLike(line.value)) items.unshift(line.value);
  }
...
function renderLineGhost(line, variables) {
  ...
  if (line.kind === 'assignment') {
    ...
    return `<span class="ghost-var">${raw.slice(0, eq).trim()}</span>${raw.slice(eq, eq + 1)}${highlightExpression(raw.slice(eq + 1), variables)}`;
  }
